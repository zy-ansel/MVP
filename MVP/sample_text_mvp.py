...
vae_ckpt, var_ckpt = 'ckpt/var/vae_ch160v4096z32.pth', f'/VAR/log2/local_output_t2i_d{MODEL_DEPTH}/ar-ckpt-best.pth'
...
if 'vae' not in globals() or 'var' not in globals():
    vae, var = build_vae_var2(
        V=4096, Cvae=32, ch=160, share_quant_resi=4,    # hard-coded VQVAE hyperparameters
        device=device, patch_nums=patch_nums,
        num_classes=1000, depth=MODEL_DEPTH, 
        shared_aln=False, outer_nums=28, text=True,
...
text_prompts = ["A gray wolf standing in a forest.",
"A dog laying on the grass.",
"A sports car parked on the street.",
"A cup of coffee on a table.",
"A hot air balloon floating in the sky.",
"A bridge over a rive."
...
for idx, text_prompt in enumerate(text_prompts):
    ...
    os.makedirs(f'results_sample/text_sample_{idx}', exist_ok=True)
    # 为每个seed创建单独的子目录
    seed_dir = f'results_sample/text_sample_{idx}/sample_{seed}'
    os.makedirs(seed_dir, exist_ok=True)

    # 保存网格图像
    output_path = f'results_sample/text_sample_{idx}/sample_seed{seed}.png'
