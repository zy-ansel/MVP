...
from utils.clip_loss import clip_contrastive_loss
...
class VARTrainer(object):
    ...
    def eval_ep(self, ld_val: DataLoader):
        ...
        for inp_B3HW, label_B, label_name, inp_clip in ld_val:
            B, V = label_B.shape[0], self.vae_local.vocab_size
            inp_B3HW = inp_B3HW.to(dist.get_device(), non_blocking=True)
            label_B = label_B.to(dist.get_device(), non_blocking=True)
            inp_clip = inp_clip.to(dist.get_device(), non_blocking=True)
            ...
            # logits_BLV, _, _, _ = self.var_wo_ddp(label_B, x_BLCv_wo_first_l)
    ...
    def train_step(
        ...
        inp_B3HW: FTen, inp_clip: FTen, label_B: Union[ITen, FTen], inp_clip_half: FTen, prog_si: int, prog_wp_it: float,
        ...
        with torch.no_grad():
            text_embedding = self.clip.encode_text(inp_clip)
            text_half_embedding = self.clip.encode_text(inp_clip_half)
        
        with self.var_opt.amp_ctx:
            self.var_wo_ddp.forward
            logits_BLV, img_half, img= self.var(label_B, x_BLCv_wo_first_l)
            # logits_BLV, img_half, img, energy_loss = self.var(label_B, x_BLCv_wo_first_l)
            ...
            img_embedding = self.clip.encode_image(img_resized)

            clip_loss = clip_contrastive_loss(img_embedding, text_embedding)
            ...
            # loss = loss + 0.15 * clip_loss + energy_loss
            loss = loss + 0.1 * clip_loss + 0.08 * clip_loss_half
