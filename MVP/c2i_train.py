...
def build_everything(args: arg_util_c2i.Args):
    ...
    # ===== 仅训练 visual prompt：先冻结其余参数 =====
    trainable_name_whitelist = {
        "visual_prompt",
        "class_emb.weight"
    ...
    for name, param in var_wo_ddp.named_parameters():
        if name in trainable_name_whitelist or name.startswith("film."):
            param.requires_grad = True
            print(f"[Trainable] {name}")
        else:
            param.requires_grad = False

    # 统计参数数量（注意不要覆盖 trainable_params 名字）
    n_trainable_params = sum(p.numel() for p in var_wo_ddp.parameters() if p.requires_grad)
    n_total_params     = sum(p.numel() for p in var_wo_ddp.parameters())
    print(f"Trainable parameters: {n_trainable_params:,d} / {n_total_params:,d} "
        f"({n_trainable_params / max(1, n_total_params):.2%})")
    ...
    # 兼容未初始化分布式时的空包装
    try:
        DDPOrNull = DDP if dist.initialized() else NullDDP
    except NameError:
        class NullDDP(torch.nn.Module):
            def __init__(self, module, *_, **__):
                super().__init__()
                self.module = module
    ...
    var = DDPOrNull(
        var_wo_ddp,
        device_ids=[dist.get_local_rank()] if dist.initialized() else None,
    ...
    print('[INIT][#para] ' + ', '.join([f'{k}={count_p(m)}' for k, m in (('VAR', var_wo_ddp),)]) + '\n')
    ...
    if not trainable_params:
        raise ValueError(
            ...
            "Check requires_grad settings and names in trainable_name_whitelist. "
            "E.g., confirm that 'visual_prompt' exists in var_wo_ddp.named_parameters()."
        )
    ...
    opt_kw = dict(lr=args.tlr, weight_decay=0.0)
    ...
    for it, (inp, label, label_name, caption) in me.log_every(start_it, iters_train, ld_or_itrt, 200, header):
        ...
        if is_save_step or is_last_step:
            ...
            if dist.is_local_master():
                ...
                if best_updated:
                    print(f"Update the best ckpt at step-{it+1}")
