...
from .var import VAR
from .var_t2i import VAR as VAR_T
...
def build_vae_var(
    ...
    # VAR args
    num_classes=1000, depth=16, shared_aln=False, attn_l2_norm=True, n_cond_embed=768,
    flash_if_available=True, fused_if_available=True, control_strength=1.0,
    init_adaln=0.5, init_adaln_gamma=1e-5, init_head=0.02, init_std=-1,    # init_std < 0: automated
    outer_nums=36, text=False,
) -> Tuple[VQVAE, VAR]:
    heads = depth
    width = depth * 64
    dpr = 0.1 * depth/24
    ...
    var_wo_ddp = VAR(
        vae_local=vae_local, n_cond_embed=n_cond_embed,
        ...
        outer_nums=outer_nums, text=text,
    ).to(device)
