...
def clip_vit_b32(pretrained=True):
    ...
    if pretrained:
        ...
        hf_home = 'https://github.com/HFAiLab/clip-gen/releases/download/v0.1.0'
        clip_ckpt = 'clip_vit_b32.pt'
        if not os.path.exists(ckpt): os.system(f'wget -P pretrained {hf_home}/{clip_ckpt}')
...
def clip_vit_l14(pretrained=True):
    if pretrained:
        clip = torch.jit.load(
        '/fs/scratch/PAS2473/MM2025/neurpis2025/VAR-CLIP/pretrained/ViT-L-14.pt',
        map_location='cpu'
    )
    else:
        clip, _, _ = open_clip.create_model_and_transforms('ViT-L-14', pretrained=None)
    return clip
