...
class VAR(nn.Module):
        ...
        for i, pn in enumerate(self.patch_nums):
            ...
            if pn == 1:
                ...
                if perimeter <= outer_nums:
                    ...
                    corner_size = (outer_nums - 4) // 8
                    for j in range(corner_size + 1):
                        level_indices.append(j)
                    for j in range(1, corner_size + 1):
                        level_indices.append(j * pn)

                    for j in range(pn - corner_size - 1, pn): 
                        level_indices.append(j)
                    for j in range(1, corner_size + 1):
                        level_indices.append(j * pn + (pn - 1))
                    ...
                    for j in range(corner_size + 1):
                        level_indices.append((pn - 1) * pn + j)
                    for j in range(pn - corner_size - 1, pn - 1):
                        level_indices.append(j * pn) 
            ...
            level_indices = sorted(list(set(level_indices)))
            print(level_indices)
    ...
    def autoregressive_infer_cfg(
        ...
        next_token_map[:B] = next_token_map[:B] + visual_prompt_tokens[:self.first_l].unsqueeze(0).expand(B, -1, -1)
        ...
        for si, pn in enumerate(self.patch_nums):   # si: i-th segment
            ...
            if si != self.num_stages_minus_1:   # prepare for next stage
                ...
                next_token_map[:B] = next_token_map[:B] + visual_prompt_tokens[cur_L:cur_L + self.patch_nums[si+1]**2].unsqueeze(0).expand(B, -1, -1)
