...
class VAR(nn.Module):
    def __init__(
            ...
            if pn == 1:
                ...
                if perimeter <= outer_nums:
                    ...
                    for j in range(corner_size + 1): # 0, 1 
                        level_indices.append(j)  # 0, 1
                    for j in range(1, corner_size + 1): # 1
                        level_indices.append(j * pn)  # 5
                    
                    for j in range(pn - corner_size - 1, pn): # 3, 4
                        level_indices.append(j)  # 3, 4
                    for j in range(1, corner_size + 1): # 1
                        level_indices.append(j * pn + (pn - 1))  # 9
                    
                    for j in range(corner_size + 1): # 0, 1
                        level_indices.append((pn - 1) * pn + j)  # 20, 21
                    for j in range(pn - corner_size - 1, pn - 1): # 3
                        level_indices.append(j * pn)  # 15
        ...
        for prompt_layer in self.prompt_layers:
            nn.init.zeros_(prompt_layer[-1].weight)
        # 8. text embedding
